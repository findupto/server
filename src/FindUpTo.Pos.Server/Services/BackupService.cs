...
public static class BackupService
{
    ...
    public static string GetDatabasePath(CoreDbContext db)
    {
        var path = db.Database.GetDbConnection().DataSource;
        if (string.IsNullOrWhiteSpace(path) || path == ":memory:")
            throw new InvalidOperationException("A file-backed SQLite database is required.");
    ...
    {
        var databasePath = GetDatabasePath(db);
        if (!File.Exists(databasePath)) throw new FileNotFoundException("Database file not found.", databasePath);
        var directory = Path.Combine(contentRootPath, "backups");
        Directory.CreateDirectory(directory);
        var stamp = DateTime.UtcNow.ToString("yyyyMMdd-HHmmssfff");
        var backupPath = Path.Combine(directory, $"pos-{stamp}.db");

        await Gate.WaitAsync(cancellationToken);
        try
        {
            await db.Database.ExecuteSqlRawAsync($"VACUUM INTO '{backupPath.Replace("'", "''")}'", cancellationToken);
        }
...
public sealed class AutomaticBackupHostedService(IServiceScopeFactory scopeFactory, IWebHostEnvironment environment, IConfiguration configuration, ILogger<AutomaticBackupHostedService> logger) : BackgroundService
{
    ...
    {
        var hours = Math.Clamp(configuration.GetValue("Backup:IntervalHours", 24), 1, 168);
        var retentionDays = Math.Clamp(configuration.GetValue("Backup:RetentionDays", 30), 1, 3650);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var file = await BackupService.CreateBackupAsync(db, environment.ContentRootPath, stoppingToken);
                BackupService.PruneBackups(environment.ContentRootPath, retentionDays);
                logger.LogInformation("Automatic POS backup created: {File}", file.FullName);
            }
