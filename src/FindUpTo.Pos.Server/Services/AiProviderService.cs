...
public sealed class AiProviderService(IHttpClientFactory httpClientFactory, CoreDbContext db)
{
    public async Task<AiProviderInfo> ResolveAsync(CancellationToken cancellationToken = default)
    {
        ...
        var ollama = await ProbeOllamaAsync(cancellationToken); if (ollama is not null) return ollama;
        var lm = await ProbeOpenAiCompatibleAsync("lmstudio", "http://127.0.0.1:1234", cancellationToken); if (lm is not null) return lm;
        var llama = await ProbeOpenAiCompatibleAsync("llamacpp", "http://127.0.0.1:8080", cancellationToken); if (llama is not null) return llama;
        var envKey = Environment.GetEnvironmentVariable("OPENAI_API_KEY")?.Trim(); if (!string.IsNullOrWhiteSpace(envKey)) return new("openai", Environment.GetEnvironmentVariable("OPENAI_BASE_URL")?.Trim().TrimEnd('/') ?? "https://api.openai.com", Environment.GetEnvironmentVariable("OPENAI_MODEL")?.Trim() ?? "gpt-5", true, false, "environment");
    ...
    {
        var settings = await db.BusinessSettings.AsNoTracking().SingleAsync(cancellationToken);
        var configured = settings.AiApiKeyEncrypted?.Trim();
        return string.IsNullOrWhiteSpace(configured) ? Environment.GetEnvironmentVariable("OPENAI_API_KEY")?.Trim() : configured;
    }
    ...
    public async Task ConfigureAsync(string provider, string model, string? apiKey, string? baseUrl, CancellationToken cancellationToken = default)
    {
        ...
        settings.AiProvider = string.IsNullOrWhiteSpace(provider) ? "auto" : provider.Trim().ToLowerInvariant(); settings.AiModel = model?.Trim() ?? ""; settings.AiBaseUrl = baseUrl?.Trim().TrimEnd('/') ?? "";
        if (apiKey is not null) settings.AiApiKeyEncrypted = apiKey.Trim(); settings.UpdatedAtUtc = DateTime.UtcNow; await db.SaveChangesAsync(cancellationToken);
    }

    private async Task<AiProviderInfo?> ProbeOllamaAsync(CancellationToken cancellationToken)
    {
        try { var client = httpClientFactory.CreateClient("business-ai"); using var response = await client.GetAsync("http://127.0.0.1:11434/api/tags", cancellationToken); if (!response.IsSuccessStatusCode) return null; var json = JsonNode.Parse(await response.Content.ReadAsStringAsync(cancellationToken)); var model = json?["models"]?.AsArray().FirstOrDefault()?["name"]?.GetValue<string>(); return string.IsNullOrWhiteSpace(model) ? null : new("ollama", "http://127.0.0.1:11434", model, false, true, "detected"); } catch { return null; }
    }
    ...
    {
        try { var client = httpClientFactory.CreateClient("business-ai"); using var response = await client.GetAsync($"{baseUrl}/v1/models", cancellationToken); if (!response.IsSuccessStatusCode) return null; var json = JsonNode.Parse(await response.Content.ReadAsStringAsync(cancellationToken)); var model = json?["data"]?.AsArray().FirstOrDefault()?["id"]?.GetValue<string>(); return string.IsNullOrWhiteSpace(model) ? null : new(name, baseUrl, model, false, true, "detected"); } catch { return null; }
    }

    public static HttpRequestMessage CreateRequest(AiProviderInfo provider, string path) => new(HttpMethod.Post, provider.BaseUrl.TrimEnd('/') + path);
}
