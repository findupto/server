...
public sealed class BusinessAiService(IHttpClientFactory httpClientFactory, CoreDbContext db, PromotionPricingService pricing, InventoryService inventory)
{
    ...
    private async Task<JsonObject> CreateResponseAsync(string apiKey, string model, JsonArray input, JsonArray tools, string username, string role, CancellationToken cancellationToken)
    {
        var instructions = $"You are FindUpTo POS Business Operator, an action-oriented restaurant/retail POS assistant. Current authenticated user: {username}; role: {role}. " +
            "You have direct access to live POS business tools. Use tools instead of guessing database values. " +
            "For financial or inventory mutations, act only when the user clearly requested the mutation; never invent products, quantities, prices, supplier names, payment amounts, or customer identity. " +
        ...
        if (!response.IsSuccessStatusCode) throw new InvalidOperationException($"AI provider error ({(int)response.StatusCode}): {content}");
    ...
    private async Task<object> SearchProductsAsync(JsonObject args)
    {
        ...
        var products = await db.Products.AsNoTracking().Where(x => x.Available && (query == "" || x.Name.Contains(query) || x.Barcode.Contains(query))).OrderBy(x => x.Name).Take(30).ToListAsync();
        var ids = products.Select(x => x.Id).ToList();
        var stock = await db.ProductInventories.AsNoTracking().Where(x => ids.Contains(x.ProductId)).ToDictionaryAsync(x => x.ProductId);
        return products.Select(p => new { p.Id, p.Name, p.Barcode, p.Price, stock = stock.GetValueOrDefault(p.Id)?.QuantityOnHand ?? 0m, averageCost = stock.GetValueOrDefault(p.Id)?.AverageCost ?? 0m, trackInventory = stock.GetValueOrDefault(p.Id)?.TrackInventory ?? false }).ToList();
    }
