...
public sealed class BusinessAiService(IHttpClientFactory httpClientFactory, CoreDbContext db, PromotionPricingService pricing, InventoryService inventory, AiProviderService providers)
{
    ...
    private async Task<object> CreateSaleAsync(JsonObject args, ClaimsPrincipal user, string username, string role, CancellationToken cancellationToken)
    {
        ...
        await using var tx = await db.Database.BeginTransactionAsync(cancellationToken); db.Orders.Add(order); await db.SaveChangesAsync(cancellationToken); var stock = await inventory.DeductForSaleAsync(order); if (!stock.Success) { await tx.RollbackAsync(cancellationToken); throw new InvalidOperationException($"Insufficient stock for {stock.ProductName}: required {stock.Required}, available {stock.Available}."); } if (!string.IsNullOrWhiteSpace(normalized)) { var change = normalized == "Cash" ? Math.Max(0m, tendered - order.Total) : 0m; db.Payments.Add(new Payment { PosOrderId = order.Id, AmountTendered = tendered, AmountPaid = order.Total, ChangeAmount = change, Method = normalized, Reference = args["payment_reference"]?.GetValue<string>()?.Trim() ?? "", CollectedByUsername = username }); } await db.SaveChangesAsync(cancellationToken); await tx.CommitAsync(cancellationToken); await AuditEndpoints.WriteAsync(db, user, "CreatedByAI", "Order", order.Id.ToString(), $"AI sale by {username}");
