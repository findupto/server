...
public sealed class PremiumIntelligenceService(CoreDbContext db)
{
    public async Task<object> GetExecutiveDashboardAsync(int days, CancellationToken ct)
    {
        ...
        var orders = await db.Orders.AsNoTracking().Where(x => x.CreatedAtUtc >= fromDate && x.Status != "Cancelled").Include(x => x.Items).ToListAsync(ct);
        var payments = await db.Payments.AsNoTracking().Where(x => x.CreatedAtUtc >= fromDate && x.Status == "Paid").ToListAsync(ct);
        var expenses = await db.Expenses.AsNoTracking().Where(x => x.ExpenseDateUtc >= fromDate).ToListAsync(ct);
        ...
        var previous = await db.Orders.AsNoTracking().Where(x => x.CreatedAtUtc >= previousFrom && x.CreatedAtUtc < fromDate && x.Status != "Cancelled").Select(x => x.Total).ToListAsync(ct);
        var previousRevenue = previous.Sum();
        return new { generatedAtUtc = now, periodDays = days, revenue, orders = orders.Count, averageTicket = orders.Count == 0 ? 0m : Math.Round(revenue / orders.Count, 2), grossProfit = Math.Round(grossProfit, 2), grossMargin = revenue == 0 ? 0m : Math.Round(grossProfit / revenue, 4), expenses = Math.Round(expenseTotal, 2), contributionAfterExpenses = Math.Round(grossProfit - expenseTotal, 2), revenueGrowth = previousRevenue == 0 ? (revenue > 0 ? 1m : 0m) : Math.Round((revenue - previousRevenue) / previousRevenue, 4), payments = payments.GroupBy(x => x.Method).Select(g => new { method = g.Key, amount = Math.Round(g.Sum(x => x.AmountPaid), 2), transactions = g.Count() }).OrderByDescending(x => x.amount).ToList(), daily };
    }
    ...
    public async Task<object> GetMenuEngineeringAsync(int days, CancellationToken ct)
    {
        ...
        var items = await (from item in db.OrderItems.AsNoTracking()
                           ...
                           select new { productId = g.Key.ProductId, productName = g.Key.ProductName, units = g.Sum(x => x.Quantity), revenue = g.Sum(x => x.LineTotal - x.DiscountAmount), grossProfit = g.Sum(x => x.LineTotal - x.DiscountAmount - x.UnitCost * x.Quantity) }).ToListAsync(ct);
        ...
        var result = items.Select(x =>
        {
            ...
            var quadrant = popular && profitable ? "Star" : popular ? "Plowhorse" : profitable ? "Puzzle" : "Dog";
    ...
    {
        var now = DateTime.UtcNow; var fromDate = now.AddDays(-days);
        ...
        {
            var o = orders.Where(x => x.CustomerId == c.Id).ToList(); var last = o.Count == 0 ? (DateTime?)null : o.Max(x => x.CreatedAtUtc);
            var recency = last.HasValue ? (now - last.Value).TotalDays : days; var frequency = o.Count; var monetary = o.Sum(x => x.Total);
    ...
    public async Task<object> GetSalesForecastAsync(int historyDays, int horizonDays, CancellationToken ct)
    {
        ...
        var values = series.Select(x => x.value).ToList(); var recent = values.TakeLast(Math.Min(7, values.Count)).DefaultIfEmpty(0m).Average(); var older = values.Take(Math.Max(1, values.Count - Math.Min(7, values.Count))).DefaultIfEmpty(0m).Average();
    ...
    {
        var products = await db.Products.AsNoTracking().ToListAsync(ct); var customers = await db.Customers.AsNoTracking().ToListAsync(ct); var inventory = await db.ProductInventories.AsNoTracking().ToListAsync(ct); var categories = await db.Categories.AsNoTracking().ToListAsync(ct);
        var categoryIds = categories.Select(x => x.Id).ToHashSet(); var inventoryByProduct = inventory.ToDictionary(x => x.ProductId);
        var missingCategory = products.Count(x => !categoryIds.Contains(x.CategoryId)); var missingCost = products.Count(x => !inventoryByProduct.TryGetValue(x.Id, out var i) || i.AverageCost <= 0); var missingBarcode = products.Count(x => string.IsNullOrWhiteSpace(x.Barcode)); var missingDescription = products.Count(x => string.IsNullOrWhiteSpace(x.Description)); var missingImage = products.Count(x => string.IsNullOrWhiteSpace(x.ImageUrl)); var missingPhone = customers.Count(x => string.IsNullOrWhiteSpace(x.Phone));
    ...
    {
        var now = DateTime.UtcNow; var fromDate = now.AddDays(-days); var alerts = new List<object>();
        ...
        var orders = await db.Orders.AsNoTracking().Where(x => x.CreatedAtUtc >= fromDate && x.Status != "Cancelled").ToListAsync(ct); var revenue = orders.Sum(x => x.Total); var average = orders.Count == 0 ? 0m : revenue / orders.Count;
        ...
        var expenses = await db.Expenses.AsNoTracking().Where(x => x.ExpenseDateUtc >= fromDate).ToListAsync(ct); var dailyExpense = expenses.Count == 0 ? 0m : expenses.Sum(x => x.Amount) / days;
    ...
    {
        var values = source.OrderBy(x => x).ToArray(); if (values.Length == 0) return 0m; var middle = values.Length / 2;
