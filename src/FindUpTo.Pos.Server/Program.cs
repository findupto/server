...
if (string.IsNullOrWhiteSpace(jwtKey) || jwtKey.Length < 32)
    throw new InvalidOperationException("A JWT signing key of at least 32 characters is required. Configure POS_JWT_KEY or Jwt:Key.");
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        OnMessageReceived = context =>
        {
            ...
            if (!string.IsNullOrEmpty(token) && context.HttpContext.Request.Path.StartsWithSegments("/hubs/pos"))
                context.Token = token;
...
static async Task SeedAsync(CoreDbContext db, IPasswordHasher<AppUser> hasher)
{
    if (!await db.BusinessSettings.AnyAsync())
    {
        ...
        {
            BusinessName = Environment.GetEnvironmentVariable("POS_BUSINESS_NAME")?.Trim() is { Length: > 0 } businessName ? businessName : "FindUpTo POS",
            Phone = Environment.GetEnvironmentVariable("POS_BUSINESS_PHONE")?.Trim() ?? "",
            Address = Environment.GetEnvironmentVariable("POS_BUSINESS_ADDRESS")?.Trim() ?? "",
            TaxPercent = decimal.TryParse(Environment.GetEnvironmentVariable("POS_TAX_PERCENT"), out var tax) ? Math.Clamp(tax, 0m, 100m) : 0m,
            CurrencyCode = Environment.GetEnvironmentVariable("POS_CURRENCY_CODE")?.Trim().ToUpperInvariant() is { Length: 3 } currencyCode ? currencyCode : "PKR",
            CurrencySymbol = Environment.GetEnvironmentVariable("POS_CURRENCY_SYMBOL")?.Trim() is { Length: > 0 } currencySymbol ? currencySymbol : "Rs."
        });
