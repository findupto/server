...
public static class AiEndpoints
{
    ...
    public static void MapAiEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/ai/customers/retention", async (CoreDbContext db, int? days, CancellationToken cancellationToken) =>
        {
            ...
            var result = customers.Select(c =>
            {
                ...
                var frequency = customerOrders.Count;
                var value = customerOrders.Sum(x => x.Total);
                var score = frequency == 0 ? 100 : Math.Clamp((decimal)recency * 0.7m - frequency * 2m - value / 100m, 0m, 100m);
                var risk = frequency == 0 ? "Inactive" : score >= 70 ? "Critical" : score >= 45 ? "High" : score >= 25 ? "Medium" : "Low";
                return new { customerId = c.Id, customerName = c.Name, phone = c.Phone, orders = frequency, spend = Math.Round(value, 2), lastOrderAtUtc = last, daysSinceLastOrder = recency, churnRiskScore = Math.Round(score, 1), churnRisk = risk, recommendedAction = risk switch { "Critical" => "WinBack", "High" => "PersonalOffer", "Medium" => "Engage", _ => "Retain" } };
        ...
        app.MapGet("/api/ai/customers/summary", async (CoreDbContext db, CancellationToken cancellationToken) =>
        {
            ...
            var orders = await db.Orders.AsNoTracking().Where(x => x.CustomerId.HasValue && x.CreatedAtUtc >= from && x.Status != "Cancelled").ToListAsync(cancellationToken);
            var active = orders.Select(x => x.CustomerId!.Value).Distinct().Count();
            var repeat = orders.GroupBy(x => x.CustomerId!.Value).Count(g => g.Count() >= 2);
    ...
    private static async Task<object> GetInventoryForecastAsync(CoreDbContext db, int horizon, CancellationToken ct)
    {
        ...
        var sales = await db.OrderItems.AsNoTracking().Where(x => x.PosOrder.CreatedAtUtc >= from && x.PosOrder.Status != "Cancelled").GroupBy(x => x.ProductId).Select(g => new { productId = g.Key, units = g.Sum(x => x.Quantity) }).ToDictionaryAsync(x => x.productId, ct);
        var items = products.Select(p => { inventory.TryGetValue(p.Id, out var s); var units = sales.GetValueOrDefault(p.Id); var daily = units / (decimal)Math.Max(30, horizon); var forecast = Math.Ceiling(daily * horizon); var onHand = s?.QuantityOnHand ?? 0m; var reorder = s?.ReorderLevel ?? 0m; return new { productId = p.Id, productName = p.Name, quantityOnHand = onHand, reorderLevel = reorder, unitsSold = units, dailySalesRate = Math.Round(daily, 3), forecastUnits = forecast, daysRemaining = daily > 0 ? Math.Round(onHand / daily, 1) : (decimal?)null, recommendedOrderQuantity = Math.Max(0m, Math.Ceiling(Math.Max(reorder, forecast) - onHand)) }; }).OrderByDescending(x => x.recommendedOrderQuantity).ToList();
    ...
    {
        var suppliers = await db.Suppliers.AsNoTracking().Where(x => x.Active).ToListAsync(ct); var orders = await db.PurchaseOrders.AsNoTracking().Where(x => x.Status != "Cancelled").Include(x => x.Items).ToListAsync(ct);
    ...
    {
        var inventory = await db.ProductInventories.AsNoTracking().Where(x => x.TrackInventory && x.QuantityOnHand <= x.ReorderLevel).ToListAsync(ct); var products = await db.Products.AsNoTracking().ToDictionaryAsync(x => x.Id, ct);
