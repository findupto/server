...
public static class DeviceEndpoints
{
    public static void MapDeviceEndpoints(this WebApplication app)
    {
        ...
        app.MapPatch("/api/relay/devices/{id:int}", async (int id, bool active, ClaimsPrincipal user, CoreDbContext db) => { var device = await db.RelayDevices.FindAsync(id); if (device is null) return Results.NotFound(); device.Active = active; await db.SaveChangesAsync(); await AuditEndpoints.WriteAsync(db, user, active ? "Activated" : "Deactivated", "RelayDevice", id.ToString(), device.Name); return Results.Ok(new { device.Id, device.Name, device.Active }); }).RequireAuthorization(p => p.RequireRole("Owner", "Manager", "Admin"));
