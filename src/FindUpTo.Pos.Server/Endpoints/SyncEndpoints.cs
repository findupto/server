    ...
    public static void MapSyncEndpoints(this WebApplication app)
    {
        ...
        app.MapPost("/api/sync/push-orders", async (List<CreateOrderRequest> orders, HttpContext context, CoreDbContext db, CancellationToken ct) =>
        {
            ...
            foreach (var input in orders)
            {
                ...
                if (existing is not null) { results.Add(new { success = true, duplicate = true, orderId = existing.OrderId }); continue; }
                if (input.Items is null || input.Items.Count == 0)
                { await RecordConflict(input.ClientOperationId, "", "EmptyOrder", "Order has no items."); results.Add(new { success = false, conflict = true, error = "Order has no items." }); continue; }
                ...
                { await RecordConflict(input.ClientOperationId, "", "ProductUnavailable", "One or more products are unavailable."); results.Add(new { success = false, conflict = true, error = "One or more products are unavailable." }); continue; }
                if (input.CustomerId.HasValue && !await db.Customers.AnyAsync(x => x.Id == input.CustomerId.Value, ct))
                { await RecordConflict(input.ClientOperationId, input.CustomerId.Value.ToString(), "CustomerMissing", "Customer not found."); results.Add(new { success = false, conflict = true, error = "Customer not found." }); continue; }
                if (input.TableId.HasValue && !await db.Tables.AnyAsync(x => x.Id == input.TableId.Value && x.Active, ct))
                { await RecordConflict(input.ClientOperationId, input.TableId.Value.ToString(), "TableUnavailable", "Table not found or inactive."); results.Add(new { success = false, conflict = true, error = "Table not found or inactive." }); continue; }
                ...
                if (order is null)
                { await RecordConflict(input.ClientOperationId, "", "InvalidLine", "Invalid quantity or product."); results.Add(new { success = false, conflict = true, error = "Invalid quantity or product." }); continue; }
