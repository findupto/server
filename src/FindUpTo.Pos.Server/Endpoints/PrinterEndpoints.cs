...
public static class PrinterEndpoints
{
    ...
    private static async Task PrintReceiptAsync(string address, PosOrder order, BusinessSetting? settings, CancellationToken ct)
    {
        ...
        text.Append("\x1B\x40").Append("\x1B\x61\x01").Append(businessName).Append("\n");
        ...
        text.Append("ORDER #").Append(order.Id).Append("\n").Append(order.CreatedAtUtc.ToLocalTime().ToString("yyyy-MM-dd HH:mm")).Append("\n").Append("\x1B\x61\x00").Append("--------------------------------\n");
        foreach (var item in order.Items) text.Append(item.ProductName).Append("\n").Append(item.Quantity).Append(" x ").Append(currency).Append(item.UnitPrice.ToString("0.00")).Append("    ").Append(currency).Append(item.LineTotal.ToString("0.00")).Append("\n");
        text.Append("--------------------------------\n").Append("Subtotal: ").Append(currency).Append(order.Subtotal.ToString("0.00")).Append("\n").Append("Tax:      ").Append(currency).Append(order.Tax.ToString("0.00")).Append("\n").Append("TOTAL:    ").Append(currency).Append(order.Total.ToString("0.00")).Append("\n\nThank you!\n\n\n\x1D\x56\x00");
    ...
    private static IEnumerable<IPAddress> LocalSubnetCandidates()
    {
        ...
        {
            var bytes = address.Address.GetAddressBytes(); var mask = address.IPv4Mask.GetAddressBytes(); if (mask.Length != 4) continue;
            var network = new byte[4]; var broadcast = new byte[4];
            for (var i = 0; i < 4; i++) { network[i] = (byte)(bytes[i] & mask[i]); broadcast[i] = (byte)(network[i] | ~mask[i]); }
            var networkValue = ToUInt32(network); var broadcastValue = ToUInt32(broadcast); var hostCount = broadcastValue > networkValue ? broadcastValue - networkValue - 1 : 0;
            if (hostCount == 0 || hostCount > 254) continue;
            for (var value = networkValue + 1; value < broadcastValue; value++) { var ip = FromUInt32(value); if (!ip.Equals(address.Address)) yield return ip; }
        }
    }
