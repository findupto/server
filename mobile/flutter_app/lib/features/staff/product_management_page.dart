...
class _ProductManagementPageState extends State<ProductManagementPage> {
  ...
  Future<void> load() async {
    try {
      final c = await widget.api.categories();
      final p = await widget.api.get('/api/products');
      if (mounted) {
        setState(() {
          categories = c;
          products = List<dynamic>.from(p);
          loading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> edit([Map<String, dynamic>? existing]) async {
    ...
    int? category = existing?['categoryId'] as int? ??
  ...
  Future<void> addCategory() async {
    ...
    if (ok == true && c.text.trim().isNotEmpty) {
      try {
        await widget.api.createCategory(c.text.trim());
        await load();
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString())));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: addCategory, icon: const Icon(Icons.category)),
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                        'Rs. ${p['price']} • ${p['available'] == true ? 'Available' : 'Unavailable'}',
                      ),
                      trailing: IconButton(
                        onPressed: () => edit(p),
