...
class CounterOrdersPage extends StatefulWidget {
  ...
  @override
  State<CounterOrdersPage> createState() => _CounterOrdersPageState();
}
...
class _CounterOrdersPageState extends State<CounterOrdersPage> {
  ...
  Future<void> load() async {
    try {
      final v = await widget.api.getList('/api/orders');
      if (mounted) {
        setState(() {
          orders = v;
          loading = false;
  ...
  Future<void> pay(Map<String, dynamic> order) async {
    ...
    try {
      ...
      await widget.api.post('/api/orders/${order['id']}/payment', {
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final paid = o['status'] == 'Paid' ||
                          o['paymentStatus'] == 'Paid';
                      return Card(
                        child: ListTile(
                          title: Text(
                            'Order #${o['id']} • ${o['orderType'] ?? 'Counter'}',
                          ),
                          subtitle: Text('${o['status']} • Rs. ${o['total']}'),
                          trailing: paid
