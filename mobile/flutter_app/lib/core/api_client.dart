...
class PosApiClient {
  PosApiClient({required this.baseUrl}) : _preferences = SharedPreferences.getInstance();
  ...
  Future<void> saveToken(String token) async {
    ...
    await preferences.setString('pos_jwt', token);
  }

  Future<String?> token() async {
    final preferences = await _preferences;
    return preferences.getString('pos_jwt');
  }

  Future<void> clearToken() async {
    final preferences = await _preferences;
    await preferences.remove('pos_jwt');
  }

  Future<void> saveCustomerAccessToken(String token) async {
    final preferences = await _preferences;
    await preferences.setString('customer_access_token', token);
  }

  Future<String?> customerAccessToken() async {
    final preferences = await _preferences;
    return preferences.getString('customer_access_token');
  }

  Future<void> clearCustomerAccessToken() async {
    final preferences = await _preferences;
    await preferences.remove('customer_access_token');
  }
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    final d = await _request('POST', '/api/auth/login', body: {'username': username, 'password': password});
  ...
  Future<Map<String, dynamic>> aiConfiguration({required String provider, String model = '', String apiKey = '', String baseUrl = ''}) async => Map<String, dynamic>.from(await put('/api/ai/configuration', {'provider': provider, 'model': model, 'apiKey': apiKey.isEmpty ? null : apiKey, 'baseUrl': baseUrl.isEmpty ? null : baseUrl}));
  Future<List<dynamic>> discoverAiProviders() async => getList('/api/ai/providers/discover');
  ...
  Future<Map<String, dynamic>> customerOrderTracking(int orderId) async => Map<String, dynamic>.from(await get('/api/customer/orders/$orderId/tracking'));
  Future<Map<String, dynamic>> trackParcel(String trackingCode) async => Map<String, dynamic>.from(await get('/api/delivery/track/${Uri.encodeComponent(trackingCode)}'));
  ...
  Future<List<dynamic>> riderList() async => getList('/api/delivery/riders');
  Future<Map<String, dynamic>> riderAvailability(bool online, {bool available = true}) async => Map<String, dynamic>.from(await post('/api/delivery/riders/me/availability', {'online': online, 'available': available}));
  Future<Map<String, dynamic>> riderLocation(double latitude, double longitude, {double? accuracy, double? speed, double? heading}) async => Map<String, dynamic>.from(await post('/api/delivery/riders/me/location', {'latitude': latitude, 'longitude': longitude, 'accuracyMeters': accuracy, 'speedMetersPerSecond': speed, 'headingDegrees': heading}));
  Future<Map<String, dynamic>> assignRider(int orderId, int riderId) async => Map<String, dynamic>.from(await post('/api/delivery/orders/$orderId/assign-rider', {'riderId': riderId}));
  ...
  Future<void> markRead(int id) async {
    await _request('POST', '/api/messages/$id/read');
  }
}
