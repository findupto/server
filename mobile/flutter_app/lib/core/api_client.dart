...
class PosApiClient {
  ...
  Future<dynamic> _request(String method, String path, {Object? body}) async {
    ...
    dynamic data;
    if (text.isNotEmpty) {
      try { data = jsonDecode(text); } catch (_) { data = text; }
    }
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(data is String ? data : 'Request failed: ${response.statusCode}');
    }
    return data;
  }

  Future<Map<String, dynamic>> createCustomerSession({String? name, String? phone, String? address}) async {
    final data = await _request('POST', '/api/customer/session', body: {'name': name, 'phone': phone, 'address': address});
  ...
  Future<Map<String, dynamic>> createOrder({required List<Map<String, dynamic>> items, String orderType = 'Pickup', String? address, String? notes}) async {
    final data = await _request('POST', '/api/customer/orders', body: {'items': items, 'orderType': orderType, 'address': address, 'notes': notes});
    return Map<String, dynamic>.from(data);
  }
