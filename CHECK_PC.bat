...
start "FindUpTo POS Server" /min "%CD%\artifacts\windows-server\FindUpTo.Pos.Server.exe"
...
start "FindUpTo POS App" "%CD%\artifacts\windows-pos-app\findupto_pos_mobile.exe"
echo.
echo LOCAL CHECK PASSED
 echo Server: http://127.0.0.1:5000/health
 echo Login: Malik / local development password
 echo Windows POS: launched for manual UI check
