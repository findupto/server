...
if errorlevel 1 (
  echo ERROR: Flutter SDK is not installed or not on PATH.
  exit /b 1
)

if not defined POS_JWT_KEY set "POS_JWT_KEY=CHANGE_THIS_BEFORE_RUNNING"
if "%POS_JWT_KEY%"=="CHANGE_THIS_BEFORE_RUNNING" (
  echo ERROR: Set POS_JWT_KEY to a random secret of at least 32 characters before building/running production.
  exit /b 1
)
